...
bool OCR::verifySizes(Mat r){
    //Char sizes 55x80 // @f Original 45x77
    float aspect=55.0f/80.0f;	// @f Original 45.0f/77.0f
    float charAspect= (float)r.cols/(float)r.rows;
    float error=0.35;
    float minHeight=35;	// @f Original:15
    float maxHeight=44;	// @f Original 28
    //We have a different aspect ratio for number 1, and it can be ~0.2
    float minAspect=0.12; //  @f 2013/10/14 Original:0.2
...
//        cout << "Aspect: "<< aspect << " ["<< minAspect << "," << maxAspect << "] "  << "Area "<< percPixels <<" Char aspect " << charAspect  << " Height char "<< r.rows << "\n";
        cout << "Aspect: "<< aspect << " ["<< fixed << setprecision(2) << minAspect << "," << fixed << setprecision(2) << maxAspect << "] "  << "Area "<< fixed << setprecision(2) << percPixels <<" Char aspect " << fixed << setprecision(2) << charAspect  << " Height char "<< fixed << setprecision(2) << r.rows;
    }

    // if(percPixels < 0.8 && charAspect > minAspect && charAspect < maxAspect && r.rows >= minHeight && r.rows < maxHeight) {
    if(percPixels < 1.0 && charAspect > minAspect && charAspect < maxAspect && r.rows >= minHeight && r.rows < maxHeight) {
	if (DEBUG) cout << " *\n";
        return true;
     } else {
	if (DEBUG) cout << "\n";
